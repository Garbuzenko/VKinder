...
def run_comand(comand, user_id):
    ...
    if key != 'none':
        ...
        if key == 'next':
            db.move_offset(user_id, 1)
            id = myApi.search(user_id, offset=db.get_offset(user_id), count=1)[0]
            [comand['attachment'], content] = myApi.get_user_data(id=id, offset=db.get_offset(user_id))
            pass
        elif key == 'previous':
            db.move_offset(user_id,-1)
            id = myApi.search(user_id, offset=db.get_offset(user_id), count=1)[0]
        ...
        if event.object.payload.get('type') in CALLBACK_TYPES:
            if event.object.payload.get('type') == 'show_snackbar':
                if  'черный' in event.object.payload.get('text'):
                    ...
                    db.new_black_id(event.object.user_id, db.get_current_user())
                    pass
                elif 'избранное' in event.object.payload.get('text'):
                    print('add_favorites')
                    db.new_favirite(event.object.user_id, db.get_current_user())
                    pass
